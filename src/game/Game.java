...
public class Game {
    ...
    public TreeMap<Integer, ArrayList<Player>> startGame() {
        ...
        gameState.initializeParade(INITIAL_PARADE_LENGTH);
    ...
    // // Process the current turn (the console is cleared and the full game state
    // is
    // // displayed)
    // gameIsOver = turn(currentPlayer, parade, deck, "Play");
    ...
    // if (bonus > 0) {
    // ui.broadcastMessage("\n" + getDisplayName(currentPlayer) + " gets " + bonus
    // + " time bonus points for a quick move!");
    // }
    ...
    // Player p = combinedPlayers.get((i + currentPlayerIndex) %
    // combinedPlayers.size());
    // turn(p, parade, deck, "Play");
    // }

    // ui.broadcastMessage("══════════════════════════════════════════════════════════════\n");
    // ui.broadcastMessage(
    // "Choose cards from your hand to discard! The remaining cards in your hand
    // will be added to your river, so choose wisely!\n");
    // ui.broadcastMessage("══════════════════════════════════════════════════════════════\n");
    // for (Player currentPlayer : combinedPlayers) {
    // Card firstDiscardedCard = null;
    // Card secondDiscardedCard = null;
    ...
    // } else {
    // firstDiscardedCard = currentPlayer.chooseCardToDiscard();
    // displayCardPlayedOrDiscarded(currentPlayer, firstDiscardedCard, "Discards");
    // secondDiscardedCard = currentPlayer.chooseCardToDiscard();
    // displayCardPlayedOrDiscarded(currentPlayer, secondDiscardedCard, "Discards");
    // }
    ...
    // ui.broadcastMessage(getDisplayName(player) + " earned " + bonus + " bonus
     ...
     * Shows:
     * - A header with "Current Turn" where the active player's name is highlighted
     * in green.
     * - Each player's river.
    ...
    // // Process the parade for cards to be removed and added to the current
    // player's
    // // river.
    // ArrayList<Card> currRiver = currentPlayer.getRiver();
    // Iterator<Card> iterator = parade.iterator();
    ...
    // // Display which cards were taken
    // if (!takenCards.isEmpty()) {
    // String message = getDisplayName(currentPlayer) + " takes the following cards
    // from the parade:";
    // String cardVisual = CardPrinter.printCardRow(takenCards, true);
    ...
    // // We don't need to send a duplicate message to the human player who is the
    // // current player
    // // since they will already receive it from the broadcast
    // } else {
    // String message = getDisplayName(currentPlayer) + " takes no cards from the
    // parade!";
    // ui.broadcastMessage(message);
    ...
    // if (!takenCards.isEmpty()) {
    // ui.broadcastMessage(getDisplayName(currentPlayer) + " takes the following
    // cards from the parade:");
    // ui.broadcastMessage(CardPrinter.printCardRow(takenCards, true));
    // } else {
    // ui.broadcastMessage(getDisplayName(currentPlayer) + " takes no cards from the
    // parade!");
    // }
    ...
    // private void displayCardPlayedOrDiscarded(Player currentPlayer, Card choice,
    // String playOrDiscard) {
    // if (playOrDiscard.equals("Play")) {
    // ui.broadcastMessage(getDisplayName(currentPlayer) + " played:");
    // ui.broadcastMessage(CardPrinter.printCardRow(Collections.singletonList(choice),
    // false));
    // } else {
    ...
    // System.out.println("│" + " ".repeat(paddingLeft) + title + "
    // ".repeat(paddingRight) + "│");
    // System.out.println("╰" + "─".repeat(consoleWidth - 2) + "╯");
    // }
